...
import type { ForgeMindAppOptions } from "../types/sdk-options.js";

export function createForgeMindApp(
  options: ForgeMindAppOptions,
): Express {
  const app = express();
  ...
  if (requestLogging) {
    app.use((req, _res, next) => {
      console.log(
        `[${serviceName}] ${req.method} ${req.originalUrl}`,
      );
